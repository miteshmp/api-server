  ...
  resource :background_tasks do
    ...
    desc "Get registration_ids based on specific subscription_type " 
     ...
     post 'delete_s3_events' do
      ...
      plans.each do |plan|
        if plan =="inactive"
          registration_ids = Device.where('plan_id =? AND plan_changed_at <=?',"inactive",Time.now - Settings.hours_stored.to_i.hours).map(&:registration_id) 
        else  
          registration_ids = Device.where(plan_id: plan).map(&:registration_id)
        end
        limit = get_limit(plan)  
        delete_objects(registration_ids,limit)
      end  
